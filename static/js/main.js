...
function toggleCardVideo(videoEl) {
    ...
    if (videoEl.paused) {
        // Pause all other card videos first
...
function togglePropertyStatus(pk) {
    var badge = document.getElementById('prop-status-' + pk);
    var row = document.getElementById('property-row-' + pk);
    fetch('/dashboard/properties/' + pk + '/toggle-status/', {
        method: 'POST',
        headers: {
    ...
    .then(function(data) {
        if (data.success) {
            ...
            if (data.status === 'rented' && row) row.style.opacity = '0.5';
            else if (row) row.style.opacity = '1';
            showToast('تم تغيير الحالة إلى: ' + data.status_display, 'success');
        }
    })
    .catch(function() { showToast('حدث خطأ', 'error'); });
}

// ==================== DELETE PROPERTY ====================
function deleteProperty(pk) {
    if (!confirm('هل أنت متأكد من حذف هذه الوحدة؟')) return;
    fetch('/dashboard/properties/' + pk + '/delete/', {
        method: 'POST',
        headers: {
            'X-CSRFToken': getCookie('csrftoken'),
            'X-Requested-With': 'XMLHttpRequest'
...
window.copyPropertyLink = function(url) {
    ...
    navigator.clipboard.writeText(fullUrl).then(function() {
        showToast('\u062a\u0645 \u0646\u0633\u062e \u0631\u0627\u0628\u0637 \u0627\u0644\u0648\u062d\u062f\u0629!', 'success');
    }).catch(function() {
...
// ==================== COVER IMAGE MODAL ====================
window.showCoverImage = function(src, title) {
    var modal = document.getElementById('coverImageModal');
    var img = document.getElementById('coverModalImg');
    var ttl = document.getElementById('coverModalTitle');
    if (modal && img) {
        img.src = src;
...
(function() {
    ...
    toggle.addEventListener('click', function() {
        ...
        localStorage.setItem('rentplay_dark', isDark ? 'true' : 'false');
