...
from .models import (
    User, Agency, Property, PropertyVideo, PropertyType, City, District, Booking,
    Review, Wishlist, Message, Notification, Waitlist, Contract, Banner,
    PropertyPanorama, PropertyAvailability, HouseRule, PropertyReport, AboutPage
)
from .forms import (
...
def filter_properties(queryset, form_data):
    ...
    if form_data.get('furnished'):
        queryset = queryset.filter(is_furnished=(form_data['furnished'] == '1'))
    return queryset
...
def apply_default_sorting(queryset):
    """Default sort: Available first (by views DESC), then Rented (by rent_end_date ASC)."""
    from django.db.models import Case, When, Value, IntegerField
    queryset = queryset.annotate(
        status_order=Case(
            When(status=Property.Status.AVAILABLE, then=Value(0)),
            When(status=Property.Status.RENTED, then=Value(1)),
            default=Value(2),
            output_field=IntegerField()
        )
    ).order_by('status_order', '-view_count', 'rent_end_date')
    return queryset
...
def index(request):
    ...
    if context is None:
        ...
        if furnished:
            properties = properties.filter(is_furnished=(furnished == '1'))

        payment_period = request.GET.get('payment_period')
        if payment_period:
            if payment_period == 'daily':
                properties = properties.filter(price_daily__isnull=False)
            elif payment_period == 'weekly':
                properties = properties.filter(price_weekly__isnull=False)
            elif payment_period == 'monthly':
                properties = properties.filter(price__isnull=False)
            elif payment_period == 'annual':
                properties = properties.filter(price_annual__isnull=False)

        # Fix 7: Apply sorting if requested, otherwise use default
        sort_param = request.GET.get('sort', '')
        if sort_param in SORT_OPTIONS:
            properties = apply_sorting(properties, sort_param)
        else:
            properties = apply_default_sorting(properties)
...
def agency_detail(request, agency_slug):
    ...
    return render(request, 'agency/agency_detail.html', {
        ...
        'about_agency': agency.about,
        'cr_number': agency.cr_number,
        'cr_file': agency.cr_file,
        'facebook_url': agency.facebook,
        'tiktok_url': agency.tiktok,
        'instagram_url': agency.instagram,
    })
...
def property_detail(request, pk):
    ...
    # Similar properties: first same agency + same district, then same district from other agencies
    similar = list(Property.objects.filter(
    ...
    if len(similar) < 4:
        needed = 4 - len(similar)
        existing_ids = [s.pk for s in similar] + [pk]
        extra = Property.objects.filter(
            district=property_obj.district,
            is_published=True,
            status=Property.Status.AVAILABLE
        ).exclude(pk__in=existing_ids).select_related('property_type', 'city')[:needed]
        similar.extend(list(extra))
    ...
    og_description = property_obj.description[:160] if property_obj.description else ''
    og_image = property_obj.main_image if hasattr(property_obj, 'main_image') and property_obj.main_image else ''
    ...
    context = {
        ...
        # Open Graph meta (v7.4)
        'og_title': og_title,
        'og_description': og_description,
        'og_image': og_image,
        'og_url': og_url,
        # House rules (v7.4)
        'house_rules': property_obj.house_rules.all(),
        # Report form (v7.4)
        'report_form': {},
    }
...
def about_us(request):
    ...
    if request.method == 'POST':
        ...
        if name and email and message_body:
            full_message = f"الاسم: {name}\nالبريد: {email}\nالجوال: {phone}\n\n{message_body}"
            recipient = about_page.support_email or settings.DEFAULT_FROM_EMAIL
            try:
                send_mail(
                    subject=f"[RENTPLAY Contact] {subject or 'رسالة تواصل'}",
                ...
                messages.error(request, 'حدث خطأ أثناء إرسال الرسالة. حاول مرة أخرى.')
        else:
...
# ==================== AGENCY ABOUT PAGE ====================
def agency_about(request, agency_slug):
    """Agency about page: shows agency.about content + agency info."""
    agency = get_object_or_404(Agency, slug=agency_slug, status=Agency.Status.ACTIVE)
...
# ==================== PROPERTY REPORT (AJAX) ====================
@require_POST
def property_report(request):
    """Submit a property report (fraud, wrong info, etc.). Returns JSON."""
...
def property_compare(request):
    ...
    return render(request, 'properties/property_compare.html', {
        ...
        'compare_col_count': properties.count(),
...
def user_register(request):
    ...
    if request.method == 'POST' and form.is_valid():
        user = form.save()
        login(request, user)
        messages.success(request, 'أهلاً بك! تم إنشاء حسابك بنجاح')
        return redirect('rentplay:index')
...
def property_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not user.is_superuser and user.agency:
                prop.agency = user.agency
            elif user.is_superuser and form.cleaned_data.get('agency'):
                prop.agency = form.cleaned_data.get('agency')
            prop.created_by = user
            prop.save()
            form.save_m2m()
            vid = request.FILES.get('video')
            if vid:
                PropertyVideo.objects.update_or_create(property_unit=prop, defaults={'video': vid})
            # Handle panorama uploads
            panoramas = request.FILES.getlist('panoramas')
            for pano in panoramas:
                PropertyPanorama.objects.create(property=prop, image=pano)
    ...
    return render(request, 'dashboard/property_form.html', {
        'form': form, 'title': 'إضافة وحدة جديدة', 'action': 'إنشاء'
    })
...
def property_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            saved_prop = form.save(commit=False)
            if user.is_superuser and form.cleaned_data.get('agency'):
                saved_prop.agency = form.cleaned_data.get('agency')
            saved_prop.save()
            form.save_m2m()
            vid = request.FILES.get('video')
            if vid:
        ...
        if not user.is_superadmin:
            form.fields.pop('agency', None)
    return render(request, 'dashboard/property_form.html', {
        'form': form, 'property': prop, 'title': 'تعديل وحدة', 'action': 'تحديث'
    })
...
def property_toggle_status(request, pk):
    """AJAX toggle property status between available/rented."""
    prop = get_object_or_404(Property, pk=pk)
    user = request.user
    ...
    # Toggle between available and rented
...
def waitlist_join(request, property_pk):
    ...
    if form.is_valid():
        ...
        # For logged-in users, check by user+property
        # For guests, check by email+property
        if user:
            waitlist, created = Waitlist.objects.get_or_create(
                user=user, property_unit=property_obj,
                defaults={'email': email, 'phone': phone, 'name': name}
            )
        else:
            waitlist, created = Waitlist.objects.get_or_create(
                user=None, email=email, property_unit=property_obj,
        ...
        if created:
            return JsonResponse({'success': True, 'message': 'سيتم إبلاغك عند توفر الوحدة.'})
        return JsonResponse({'success': True, 'message': 'أنت بالفعل في قائمة الانتظار.'})
