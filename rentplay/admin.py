...
from .models import (
    ...
    PropertyPanorama, PropertyAvailability,
    HouseRule, PropertyReport, AboutPage
)
...
class PropertyPanoramaInline(admin.TabularInline):
    """Inline admin for 360° panoramas."""
    model = PropertyPanorama
    extra = 0


class PropertyAvailabilityInline(admin.TabularInline):
    """Inline admin for availability dates."""
...
class AgencyAdmin(admin.ModelAdmin):
    ...
    search_fields = ['name', 'phone', 'email', 'cr_number']
    prepopulated_fields = {'slug': ('name',)}
    readonly_fields = ['created_at', 'updated_at']
    fieldsets = (
        (_('Basic Info'), {
            'fields': ('name', 'slug', 'description', 'logo', 'admin')
        }),
        (_('Contact'), {
            'fields': ('phone', 'whatsapp', 'email')
        }),
        (_('Address & Online'), {
            'fields': ('address', 'office_address', 'website', 'city', 'district', 'map_link', 'lat', 'lng')
        }),
        (_('About & Registration'), {
            'fields': ('about', 'cr_number', 'cr_file')
        }),
        (_('Social Media'), {
            'fields': ('facebook', 'tiktok', 'instagram'),
            'classes': ('collapse',)
        }),
        (_('Status'), {
...
class PropertyAdmin(admin.ModelAdmin):
    ...
    list_display = ['unit_code', 'title', 'agency', 'property_type', 'city', 'district', 'price_annual', 'price', 'status', 'is_furnished', 'is_published', 'created_at']
    list_filter = ['status', 'is_furnished', 'is_published', 'property_type', 'city', 'district', 'created_at']
    ...
    def get_inlines(self, request, obj=None):
        if request.user.is_superuser:
            return [PropertyImageInline, PropertyVideoInline, PropertyPanoramaInline, PropertyAvailabilityInline]
        return [PropertyVideoInline, PropertyPanoramaInline, PropertyAvailabilityInline]
    ...
    fieldsets = (
        ...
        (_('Contact & Location'), {
            'fields': ('phone', 'whatsapp', 'map_link', 'street_name', 'building_number', 'postal_code', 'lat', 'lng'),
            'classes': ('collapse',)
        }),
        (_('Services & Rules'), {
            'fields': ('nearby_services', 'house_rules', 'is_price_negotiable'),
            'classes': ('collapse',)
        }),
        (_('Features'), {
            'fields': ('features',),
...
class AboutPageAdmin(admin.ModelAdmin):
    """About page content - singleton pattern."""
...
class HouseRuleAdmin(admin.ModelAdmin):
    """House rules managed by superadmin."""
...
class PropertyReportAdmin(admin.ModelAdmin):
    """Property reports - visible to agency admin and superadmin only."""

    list_display = ['property', 'reporter_name', 'reporter_phone', 'status', 'created_at']
    list_filter = ['status', 'created_at']
    search_fields = ['property__title', 'reporter_name', 'reporter_phone', 'message']
    readonly_fields = ['created_at', 'updated_at']
...
# PropertyImage removed in v4.0 - no images allowed
admin.site.register(PropertyVideo)
admin.site.register(Wishlist)
admin.site.register(Message)
admin.site.register(Waitlist)
admin.site.register(PropertyPanorama)
